...
def get_or_upload_file(client, filepath):
    """Uploads file only if it doesn't already exist in Gemini."""
...
def generate_only(prompt, path_a, path_c, job_id, style, audio, neg, guidance, motion):
    ...
    try:
        if Settings.GCP_PROJECT_ID:
            ...
            op = client.models.generate_videos(model='veo-3.1-generate-preview', prompt=full_prompt, config=types.GenerateVideosConfig(number_of_videos=1))
            while not op.done:
                time.sleep(5)
